...
[ExecuteInEditMode]
[AddComponentMenu("Tools/UGUI/ScrollRectTable")]
...
public class ScrollRectTable : MonoBehaviour
{
	...
	void doDataRemove(object data,int index)
    {
        if (onDataRemove == null)
            onDataRemove = (LuaFunction)LuaState.main.doString(DataRemoveStr, "onDataRemove");
        onDataRemove.call(this.data, index, this);
    }

    void doDataInsert(object item,int index)
    {
        if (onDataInsert == null)
            onDataInsert = (LuaFunction)LuaState.main.doString(DataInsertStr, "onDataInsert");
        onDataInsert.call(item, index, this);
    }

    void doPreRender(object item,int index,object data)
    {
        if (onPreRender == null)
            onPreRender = (LuaFunction)LuaState.main.doString(PreRenderStr, "onPreRenderStr");
        onPreRender.call(item, index, data);
    }
